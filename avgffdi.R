...
for(station in 1:length(stdata$FFDI)){
	...
	alldays = sqlQuery(biomass,paste("SELECT sum(\"IDMA\") from meteorology.fire_weather_ibra where stnum = ",stdata$stnum[station],";",sep=""))
	if(length(alldays$sum[1])==0){next}
	stdata$FFDI[station]=alldays$sum[1]
	print("Done")
}
...
for(station in 1:length(bigframe$stationlist)){
	...
	ffdi = as.numeric(sum(alldays$IDMA[alldays$stnum == bigframe$stationlist[station]]))
